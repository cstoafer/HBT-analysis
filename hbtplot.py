...
def get_puff_pressures(thistree):
	...
	p_IG_smth = signalAnalysis.smooth(p_IG_rawsig,250)
...
#    fig_name_list = ['q','TAn1','FB4n1','biasv','biasc','lv','mid_sxr','sxrfan','macht','machp','isat','tipEv]
    ...
    shot_data = shotData(shotno, startTime=options.zoom_start, endTime=options.zoom_end, lowPassMach=6.0)

    if 'biasv' in fig_name_list:
        shot_data.bias_v.plot(ax=ax_arr[biasv_num], color='k')

    if 'biasc' in fig_name_list:
        shot_data.bias_cur.plot(ax=ax_arr[biasc_num], color='k')
...
def plot_shot(shotno, color, shot_data, SSet, ip_ax, r_ax, q_ax, oh_ax, amp_ax, freq_ax, lv_ax, sp_ax, sxr_ax, last_shot,
    ...
    if not options.short_title:
        ip_ax.plot(shot_data.times*1e3, shot_data.Ip.data, label='%d, $p_{F}$ = %d$\mu$T' % (shot_data.shotno,pfill/1000.), color=color)
    else:
        ip_ax.plot(shot_data.times*1e3, shot_data.Ip.data, label='%d' % (shot_data.shotno), color=color)
        #ip_ax.plot(times*1e3, ip/1e3, label='%d' % shotno, color=color)

    shot_data.OH_cur.plot(ax=oh_ax, color=color, multiplier=1e-3)

    #(times, vf) = read_data(tree, '.sensors:vf_current', t_start=0,
    #                        t_end=options.end_time)
    #vf_ax.plot(times*1e3, vf*1e-3, color=color)

    if shot_data.Ip.data.max() > 10:
        shot_data.MR.plot(ax=r_ax, color=color, multiplier=1e2)

    if shot_data.Ip.data.max() > 10:
        shot_data.q.plot(ax=q_ax, color=color)

    if options.rot_probes:
        try:
            shot_data.bias_v.plot(ax=biasv_ax, color=color)
            shot_data.bias_cur.plot(ax=biasc_ax, color=color)

            shot_data.machT.plot(ax=machT_ax, color=color)
            shot_data.machP.plot(ax=machP_ax, color=color)
            shot_data.isat.plot(ax=isat_ax, color=color, multiplier=1e3)
            shot_data.tipE_v.plot(ax=tipEv_ax, color=color)
        except:
            if options.debug:
                raise
            pass
...
#    m3_ax.plot(times*1e3, m3, color=color)
    ...
    shot_data.LV.plot(ax=lv_ax, color=color)

    if shotno >= 70000:
        shot_data.DAlpha.plot(ax=sp_ax, color=color)
        if last_shot:
            spec_max = np.amax(shot_data.DAlpha.data)
            sp_ax.set_ylim(0,1.2*spec_max)

    shot_data.sxrmid.plot(ax=sxr_ax, color=color)

    if amp_ax is not None:
        # Mode amplitude and frequency using feedback sensors 4
        shot_data.FBamp.plot(ax=amp_ax, color=color)
        shot_data.FBfreq.plot(ax=freq_ax, color=color)
    ...
    avemask = times > 1.5e-3
    sig_max = np.abs(shot_data.sxrfan.data[:,avemask]).max()
    sig_min = 0.03*sig_max
    if options.white:
        sxfan_ax.contourf(times*1e3,np.arange(15)+1,shot_data.sxrfan.data,128,cmap=sxr_rainbow_colormap_w(128),
            norm=Normalize(vmin=sig_min, vmax=sig_max))
    else:
        sxfan_ax.contourf(times*1e3,np.arange(15)+1,shot_data.sxrfan.data,128,cmap=sxr_rainbow_colormap(128),
            norm=Normalize(vmin=sig_min, vmax=sig_max))


    windowLen=720
    fitExtend=0.7
    fitmask = (times > (options.zoom_start - fitExtend)/1000) & (times < (options.zoom_end + fitExtend)/1000)
    ...
    # Toroidal Array ----------------------------------------------------------------------
    shot_data.TAp.plotContour(startTime=options.zoom_start, endTime=options.zoom_end, ax=tap_ax, windowLen=windowLen, fitExtend=fitExtend)
    ...
    # Poloidal Array 1  -----------------------------------------------------------------------
    shot_data.PA1p.plotContour(startTime=options.zoom_start, endTime=options.zoom_end, ax=pa1_ax, windowLen=windowLen, fitExtend=fitExtend)
    ...
    # Poloidal Array 2  -----------------------------------------------------------------------
    shot_data.PA2p.plotContour(startTime=options.zoom_start, endTime=options.zoom_end, ax=pa2_ax, windowLen=windowLen, fitExtend=fitExtend)
    ...
    shot_data.FB4p.plotContour(startTime=options.zoom_start, endTime=options.zoom_end, ax=fb4_ax, windowLen=windowLen, fitExtend=fitExtend)
...
def main(SSet, shot_data_dict, fig, figTA, figPA1, figPA2, figFB, figTA_n1, figFB_n1):
    ...
    # Clean up saved shot data
    current_shotnos = [ shotno + current_shot if shotno <= 0 else shotno for shotno in options.shotnos ]
    for prev_shotnum in shot_data_dict.keys():
        if prev_shotnum not in current_shotnos:
            del shot_data_dict[prev_shotnum]
    ...
    for (i, shotno) in enumerate(options.shotnos):
        ...
        try:
            ...
            plot_shot(shotno, get_color(idx), shot_data, SSet, ip_ax, r_ax, q_ax, oh_ax, amp_ax, freq_ax, lv_ax, sp_ax, sxr_ax, last_shot,
...
if __name__ == '__main__':
    ...
    # Dictionary to contain a shotData instance for each shot number
    shot_data_dict = {}
    ...
    if options.loop:
        ...
        def loop():
            while True:
                ...
                gobject.idle_add(main, SSet, shot_data_dict, fig, None, None, None, None, None, None)
    ...
    main(SSet, shot_data_dict, fig, figTA, figPA1, figPA2, figFB, figTA_n1, figFB_n1)
